      ...
      <div v-loading="loading">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="版本代码">
            {{ taskDetail.versionCode || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="任务状态">
            <el-tag :type="getStatusType(taskDetail.status)">{{ getStatusText(taskDetail.status) }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="签名类型">
            {{ taskDetail.signType || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="密钥库文件">
            {{ taskDetail.keystoreFile || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="密钥别名">
            {{ taskDetail.keyAlias || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="包名">
            {{ taskDetail.packageName || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="渠道信息">
            {{ taskDetail.channelInfo || '-' }}
          </el-descriptions-item>
...
const fetchTaskDetail = async () => {
  loading.value = true
  try {
    const response = await androidTaskApi.getTaskDetail(route.params.id)
...
const handleRetry = async () => {
  ...
  try {
    const response = await androidTaskApi.retryTask(taskDetail.value.id)
